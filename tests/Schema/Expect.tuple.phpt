...
test('without items', function () {
	...
	// unlike a structure, an empty block is not a tuple of defaults
	checkValidationErrors(function () use ($schema) {
		(new Processor)->process($schema, null);
	}, ['The item expects to be array, null given.']);

	Assert::null((new Processor)->process(Expect::anyOf($schema, null), null));
});
...
test('with items', function () {
	...
	// no layer at all is null, which a tuple refuses just as a scalar type does
...
test('rest elements via otherItems()', function () {
	$schema = Expect::tuple([Expect::string()])->otherItems('int');

	Assert::equal(['a', 1, 2], (new Processor)->process($schema, ['a', 1, 2]));

	checkValidationErrors(function () use ($schema) {
		(new Processor)->process($schema, ['a', 'b']);
	}, ["The item '1' expects to be int, 'b' given."]);
});


test('describe() and JSON Schema export', function () {
	$schema = Expect::tuple([Expect::string(), Expect::int()->nullable()]);

	Assert::same(Nette\Schema\Kind::Tuple, $schema->describe()['kind']);
	...
	Assert::same(
		...
		JsonSchema::export(Expect::tuple([Expect::string()])->otherItems('int')),
	);
});
