...
test('int & min', function () {
	...
	}, ['The option expects to be in range 10.., 9 given.']);
});
...
test('int & max', function () {
	...
	}, ['The option expects to be in range ..20, 21 given.']);
});
...
test('int & min & max', function () {
	...
	checkValidationErrors(function () use ($schema) {
		(new Processor)->process($schema, 9);
	}, ['The option expects to be in range 10..20, 9 given.']);

	checkValidationErrors(function () use ($schema) {
		(new Processor)->process($schema, 21);
	}, ['The option expects to be in range 10..20, 21 given.']);
});
...
test('string', function () {
	...
	}, ['The length of option expects to be in range 1..5, 0 bytes given.']);

	checkValidationErrors(function () use ($schema) {
		(new Processor)->process($schema, 'foobar');
	}, ['The length of option expects to be in range 1..5, 6 bytes given.']);
});
...
test('unicode', function () {
	...
	}, ['The length of option expects to be in range 2..4, 1 characters given.']);

	checkValidationErrors(function () use ($schema) {
		(new Processor)->process($schema, 'žšáéx');
	}, ['The length of option expects to be in range 2..4, 5 characters given.']);
});
...
test('array', function () {
	...
	checkValidationErrors(function () use ($schema) {
		(new Processor)->process($schema, []);
	}, ['The length of option expects to be in range 1..3, 0 items given.']);

	checkValidationErrors(function () use ($schema) {
		(new Processor)->process($schema, [1, 2, 3, 4]);
	}, ['The length of option expects to be in range 1..3, 4 items given.']);
});
