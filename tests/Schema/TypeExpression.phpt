...
test('unions and composite names', function () {
	...
	Assert::same([Kind::Int, Kind::Float], kinds(TypeExpression::parse('number')));
	Assert::same([Kind::Bool, Kind::Int, Kind::Float, Kind::String], kinds(TypeExpression::parse('scalar')));
	Assert::same([Kind::Int, Kind::Float, Kind::String], kinds(TypeExpression::parse('number|string')));
});
...
test('T[] is any iterable of T, as Validators::is() sees it', function () {
	...
	Assert::same(Kind::Union, TypeExpression::parse('number[]')['items']['kind']);
