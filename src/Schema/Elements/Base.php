	...
	private function doValidateRange($value, array $range, Context $context, string $types = ''): bool
	{
		if (is_array($value) || is_string($value)) {
			...
			if (!self::isInRange($length, $range)) {
				$context->addError(
					"The length of option %path% expects to be in range %expected%, %length% $label given.",
			...
			$context->addError(
				'The option %path% expects to be in range %expected%, %value% given.',
				Nette\Schema\Message::OUT_OF_RANGE,
